...
const checks = [
  ...
  ['project collection route', server.includes("const collections=['projects','assistants','artifacts']") && server.includes("const route=`/api/${c}`")],
  ['assistant collection route', server.includes("const collections=['projects','assistants','artifacts']") && server.includes("c.slice(0,-1)")],
  ...
  ['secret omitted from account list', store.includes('withoutSecret') && store.includes('listAccounts() { return (await readStore()).accounts.map(withoutSecret)')],
